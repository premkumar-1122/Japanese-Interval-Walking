...
class OnboardingPrefs(private val context: Context) {
    ...
    companion object {
        ...
        val SEEDED_SESSION_0 = booleanPreferencesKey("seeded_session_0")
    ...
    val hasSeededSession0: Flow<Boolean> = context.onboardingDataStore.data
        .catch { exception ->
            if (exception is IOException) {
                Log.e(TAG, "Error reading preferences", exception)
                emit(emptyPreferences())
        ...
        .map { preferences ->
            preferences[SEEDED_SESSION_0] ?: false
        }
    ...
    suspend fun setSeededSession0(seeded: Boolean) {
        try {
            context.onboardingDataStore.edit { preferences ->
                preferences[SEEDED_SESSION_0] = seeded
            }
        } catch (e: Exception) {
            Log.e(TAG, "Failed to write seeded session 0 state", e)
        }
    }
