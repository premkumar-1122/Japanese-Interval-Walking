...
class WalkingForegroundService : Service(), SensorEventListener, TextToSpeech.OnInitListener {
    ...
    companion object {
        ...
        const val ACTION_ACTIVITY_RECOGNITION = "com.example.action.ACTIVITY_RECOGNITION"
    ...
    private fun stopSession(saveToDb: Boolean) {
        ...
        if (saveToDb && record.steps > 0) {
            serviceScope.launch {
                ...
                com.example.service.HealthConnectSyncWorker.enqueueSync(applicationContext)
            }
        }
